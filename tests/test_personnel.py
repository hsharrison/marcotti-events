...
def test_person_generic_insert(session, person_data):
    ...
    assert record.official_name == "John Doe"
    assert record.age(date(2010, 1, 1)) == 30
    assert record.exact_age(date(2010, 3, 15)) == (30, 74)
    assert repr(record) == "<Person(name=John Doe, country=Portlandia, DOB=1980-01-01)>"
...
def test_person_middle_naming_order(session, person_data):
    ...
    persons = [mcp.Persons(**data) for key, records in list(person_data.items())
    ...
    assert person_from_db.full_name == "Miguel Ángel Ponce"
    assert person_from_db.official_name == "Miguel Ángel Ponce Briseño"
...
def test_person_eastern_naming_order(session, person_data):
    ...
    assert person_from_db.full_name == "Son Heung-Min"
...
def test_person_nickname(session, person_data):
    ...
    assert person_from_db.full_name == "Cristiano Ronaldo"
    assert person_from_db.official_name == "Cristiano Ronaldo Aveiro dos Santos"
...
def test_person_missing_first_name_error(session, person_data):
    ...
    generic_data_without_first = {key: value for key, value in list(person_data['generic'].items()) if key != 'first_name'}
...
def test_person_missing_last_name_error(session, person_data):
    ...
    generic_data_without_last = {key: value for key, value in list(person_data['generic'].items()) if key != 'last_name'}
...
def test_person_missing_birth_date_error(session, person_data):
    ...
    generic_data_without_dob = {key: value for key, value in list(person_data['generic'].items()) if key != 'birth_date'}
...
def test_position_insert(session):
    ...
    assert repr(position_from_db) == "<Position(name=Left full-back, type=Defender)>"
...
def test_player_insert(session, person_data, position_data):
    player_data = [data for key, records in list(person_data.items()) for data in records if key in ['player']]
...
def test_player_representation(session, person_data, position_data):
    ...
    korean_player = session.query(mcp.Players).join(mco.Countries).\
        filter(mco.Countries.name == "Korea Republic").one()
    assert repr(korean_player) == "<Player(name=Son Heung-Min, DOB=1992-07-08, " \
                                  "country=Korea Republic, position=Second striker)>"

    ronaldo = session.query(mcp.Players).filter(mcp.Players.nick_name == "Cristiano Ronaldo").one()
    assert repr(ronaldo) == "<Player(name=Cristiano Ronaldo, DOB=1985-02-05, " \
                            "country=Portugal, position=Forward)>"

    mexican_player = session.query(mcp.Players).join(mco.Countries).filter(mco.Countries.name == "Mexico").one()
    assert str(mexican_player) == "<Player(name=Miguel Ángel Ponce, DOB=1989-04-12, " \
                                      "country=Mexico, position=Left back)>"


def test_manager_insert(session, person_data):
    manager_data = [mcp.Managers(**data) for key, records in list(person_data.items())
...
def test_manager_representation(session, person_data):
    ...
    zico = session.query(mcp.Managers).filter(mcp.Managers.full_name == "Zico").one()
    assert repr(zico) == "<Manager(name=Zico, DOB=1953-03-03, country=Brazil)>"

    wenger = session.query(mcp.Managers).filter(mcp.Managers.last_name == "Wenger").one()
    assert str(wenger) == "<Manager(name=Arsène Wenger, DOB=1949-10-22, country=France)>"


def test_referee_insert(session, person_data):
    referees = [mcp.Referees(**data) for key, records in list(person_data.items())
...
def test_referee_representation(session, person_data):
    ...
    english_referee = session.query(mcp.Referees).join(mco.Countries).filter(mco.Countries.name == "England").one()
    assert str(english_referee) == "<Referee(name=Christopher Foy, DOB=1962-11-20, country=England)>"

    turkish_referee = session.query(mcp.Referees).join(mco.Countries).filter(mco.Countries.name == "Turkey").one()
    assert str(turkish_referee) == "<Referee(name=Cüneyt Çakır, DOB=1976-11-23, country=Turkey)>"
...
def test_player_history_insert(session, person_data, player_history_data):
    ...
    history_from_db = session.query(mcp.PlayerHistory).join(mcp.Players).\
        filter(mcp.Players.last_name == "Doe")
...
def test_player_history_representation(session, person_data, player_history_data):
    ...
    history_from_db = session.query(mcp.PlayerHistory).join(mcp.Players).\
        filter(mcp.Players.last_name == "Doe", mcp.PlayerHistory.date == date(1998, 7, 15)).one()

    assert repr(history_from_db) == "<PlayerHistory(name=John Doe, date=1998-07-15, height=1.74, weight=76)>"
